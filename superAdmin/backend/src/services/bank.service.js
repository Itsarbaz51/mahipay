...
export class BankDetailService {
  ...
  // ONLY GET ADMINS AND THERE CHILEDERNS
  static async getbyAllBanksAdmin(params) {
    const {
      userId,
      role,
      status,
    ...
    if (user.role.name === "ADMIN" || user.role.type === "employee") {
      ...
      where.user = {
        role: { is: { name: { not: "ADMIN", not: "SUPER ADMIN" } } }, // Don't show other admin users' banks
      };
    } else {
  ...
  // super admin manage
  static async getbyAllBanksSuperAdmin(params) {
    const {
      userId,
      status = "ALL",
      page = 1,
    ...
    // Fetch SUPER ADMIN user with ADMIN children
    ...
    if (!superAdminUser) {
      throw ApiError.unauthorized("Super Admin user not found");
    }
    ...
    // SUPER ADMIN ke ADMIN children ke IDs
    const targetUserIds = superAdminUser.children.map((child) => child.id);
    ...
    if (targetUserIds.length === 0) {
      const emptyResult = {
        ...
        meta: {
          ...
          total: 0,
          totalPages: 0,
          userRole: superAdminUser.role.name,
          userRoleType: superAdminUser.role.type,
          isEmployeeViewingAllData: superAdminUser.role.type === "employee",
        },
      };
      return emptyResult;
    }
    ...
    // ✅ ADMIN service ke jaise direct banks array return karo
    const result = {
      banks,
      meta: {
        page: pageNum,
