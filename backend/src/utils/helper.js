...
class Helper {
  ...
  static async createAuthAuditLog(user, action, req, metadata = {}) {
    ...
    const logData = {
      ...
      description: Helper.getAuthActionDescription(action, user),
      ipAddress: ip,
  ...
  static async sendPasswordResetEmail(user, token, userType) {
    ...
    const { sendPasswordResetEmail } = await import(
      "./sendCredentialsEmail.js"
    );
    await sendPasswordResetEmail(user, resetUrl, userType);
  }

  static async updateRootPassword(userId, newPassword) {
  ...
  static async updateEmployeePassword(userId, newPassword) {
    const encryptedPassword = CryptoService.encrypt(newPassword);
  ...
  static async updateUserPin(userId, newPin) {
    const hashedPin = CryptoService.encrypt(newPin);
